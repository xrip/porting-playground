...
//                printf("SPC CTRL=%02X\n",v);
        ...
        case 0xF8:
        case 0xF9:
            return; /*Mortal Kombat writes to F0 and F9*/
        case 0xFA:
            spclatch[0] = v;
            return;
        case 0xFB:
...
void dumpspcregs() {
    ...
    printf("%i ins  %04X %04X %04X %04X %04X %04X %04X %04X\n", spcins, spc.opc[0], spc.opc[1], spc.opc[2], spc.opc[3],
           spc.opc[4], spc.opc[5], spc.opc[6], spc.opc[7]);
}
...
/*BEQ*/ void sopf0() {
    tmps = (signed char) readspc(spc.pc);
    spc.pc++;
    if (spc.p.z) {
        spc.pc += tmps;
        cycs = 4;
...
/*MOV dpx,A*/   void sopd4() {
    getdpx();
    writespc(addr, spc.ya.b.a);
    cycs = 5;
}

/*MOV abs,A*/   void sopc5() {
    getabs();
    writespc(addr, spc.ya.b.a);
    cycs = 5;
}

/*MOV absx,A*/  void sopd5() {
    getabs();
    addr += spc.x;
    writespc(addr, spc.ya.b.a);
    cycs = 5;
...
/*MOV dpy,X*/   void sopd9() {
    getdpy();
    writespc(addr, spc.x);
    cycs = 5;
}

/*MOV abs,X*/   void sopc9() {
    getabs();
    writespc(addr, spc.x);
    cycs = 5;
}
...
/*MOV dpx,Y*/   void sopdb() {
    getdpx();
    writespc(addr, spc.ya.b.y);
    cycs = 5;
}

/*MOV abs,Y*/   void sopcc() {
    getabs();
    writespc(addr, spc.ya.b.y);
    cycs = 5;
}
...
/*AND dp,dp*/   void sop29() {
    getdp();
    tempw = addr;
    getdp();
    temp = readspc(tempw) & readspc(addr);
    writespc(addr, temp);
    setspczn(temp);
    cycs = 6;
}
...
/*MOVW YA,dp*/  void sopba() {
    getdp();
    spc.ya.b.a = readspc(addr);
    spc.ya.b.y = readspc(addr + 1);
    setspczn16(spc.ya.ya);
    cycs = 5;
}

/*MOVW dp,YA*/  void sopda() {
    getdp();
    writespc(addr, spc.ya.b.a);
...
/*CALL*/        void sop3f() {
    getabs();
    writespc(0x100 | spc.s, spc.pc >> 8);
    spc.s--;
    writespc(0x100 | spc.s, spc.pc & 0xFF);
    spc.s--;
    spc.pc = addr;
    cycs = 8;
}
...
/*RET*/         void sop6f() {
    spc.s++;
    addr = readspc(0x100 | spc.s);
    spc.s++;
    addr |= readspc(0x100 | spc.s) << 8;
    spc.pc = addr;
    cycs = 5;
}

/*PUSH A*/      void sop2d() {
    spuram[0x100 | spc.s] = spc.ya.b.a;
    spc.s--;
    cycs = 4;
}

/*PUSH X*/      void sop4d() {
    spuram[0x100 | spc.s] = spc.x;
    spc.s--;
    cycs = 4;
}

/*PUSH Y*/      void sop6d() {
    spuram[0x100 | spc.s] = spc.ya.b.y;
    spc.s--;
    cycs = 4;
}
...
/*POP A*/       void sopae() {
    spc.s++;
    spc.ya.b.a = spuram[0x100 | spc.s];
    cycs = 4;
}

/*POP X*/       void sopce() {
    spc.s++;
    spc.x = spuram[0x100 | spc.s];
    cycs = 4;
}

/*POP Y*/       void sopee() {
    spc.s++;
    spc.ya.b.y = spuram[0x100 | spc.s];
    cycs = 4;
}

/*MUL YA*/      void sopcf() {
    spc.ya.ya = spc.ya.b.a * spc.ya.b.y;
    setspczn16(spc.ya.ya);
    cycs = 9;
}
...
/*DIV YA,X*/    void sop9e() {
    if (!spc.x) {
        ...
        spc.p.v = 0;
        temp = spc.ya.ya / spc.x;
        spc.ya.b.y = spc.ya.ya % spc.x;
        spc.ya.b.a = temp;
        setspczn(spc.ya.b.a);
    }
    cycs = 12;
...
/*ADDW YA,dp*/  void sop7a() {
    getdp();
    tempw = readspc(addr);
    tempw |= readspc(addr + 1) << 8;
    templ = spc.ya.ya + tempw;
    spc.p.v = (!((spc.ya.ya ^ tempw) & 0x8000) && ((spc.ya.ya ^ templ) & 0x8000));
    spc.ya.ya = templ & 0xFFFF;
    setspczn16(spc.ya.ya);
    spc.p.c = templ & 0x10000;
    cycs = 5;
}

/*SUBW YA,dp*/  void sop9a() {
    getdp();
    tempw = readspc(addr);
    tempw |= readspc(addr + 1) << 8;
    templ = spc.ya.ya - tempw;
    spc.p.v = (((spc.ya.ya ^ tempw) & 0x8000) && ((spc.ya.ya ^ templ) & 0x8000));
    spc.ya.ya = templ & 0xFFFF;
    spc.p.c = (spc.ya.ya >= tempw);
    setspczn16(spc.ya.ya);
    cycs = 5;
}

/*CMPW YA,dp*/  void sop5a() {
    getdp();
    tempw = readspc(addr);
    tempw |= readspc(addr + 1) << 8;
    spc.p.c = (spc.ya.ya >= tempw);
    setspczn16(spc.ya.ya - tempw);
    cycs = 4;
}
...
/*MOV1 mem.bit,C*/ void sopca() {
    getabs();
    tempw = addr >> 13;
    addr &= 0x1FFF;
    temp = readspc(addr);
    if (spc.p.c) temp |= (1 << tempw); else temp &= ~(1 << tempw);
    writespc(addr, temp);
    cycs = 6;
}

/*EOR1 C,mem.bit*/ void sop8a() {
    getabs();
    tempw = addr >> 13;
    addr &= 0x1FFF;
    temp = readspc(addr);
    if (spc.p.c) spc.p.c = 1;
    spc.p.c ^= ((temp & (1 << tempw)) ? 1 : 0);
    cycs = 4;
}

/*NOT1 mem.bit*/   void sopea() {
    getabs();
    tempw = addr >> 13;
    addr &= 0x1FFF;
    temp = readspc(addr);
...
/*POP PSW*/      void sop8e() {
    spc.s++;
    temp = spuram[0x100 | spc.s];
    spc.p.c = temp & 1;
    spc.p.z = temp & 2;
    spc.p.h = temp & 8;
    spc.p.p = (temp & 32) ? 0x100 : 0;
    spc.p.v = temp & 64;
    spc.p.n = temp & 128;
    cycs = 4;
}
