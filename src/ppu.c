...
uint32_t ppu_palette[256];
int ppu_paldirty[256];
uint32_t ppu_coltable[8], ppu_coltable16[8];
uint32_t ppu_bittable[2][16], ppu_masktable[2][16];
int ppu_firstread;
uint32_t ppu_wramaddr;
unsigned char ppu_cgwsel, ppu_cgadsub;
uint32_t ppu_fixedcol;
    ...
    for (c = 0; c < 4096; c++) {
        if (tileupdate[c]) {
            for (y = 0; y < 8; y++) {
                ...
                tiless[c][0][y][1] = ppu_bittable[0][ppu_vram[(c << 4) | (y << 1)] & 15] |
                                     (ppu_bittable[0][ppu_vram[(c << 4) | (y << 1) | 1] & 15] << 1);
                tiless[c][1][y][0] = ppu_bittable[1][ppu_vram[(c << 4) | (y << 1)] >> 4] |
                                     (ppu_bittable[1][ppu_vram[(c << 4) | (y << 1) | 1] >> 4] << 1);
                tiless[c][1][y][1] = ppu_bittable[1][ppu_vram[(c << 4) | (y << 1)] & 15] |
                                     (ppu_bittable[1][ppu_vram[(c << 4) | (y << 1) | 1] & 15] << 1);
                ...
                if (dma.ctrl[c] & 0x40) {
                    ...
                    switch (dma.ctrl[c] & 7) {
                        ...
                        case 1: /*Two regs write once*/
                            hdma.hdmaval[c] =
                                    readf(hdma.ibank[c], hdma.ta[c]) | (readf(hdma.ibank[c], hdma.ta[c] + 1) << 8);
                            hdma.ta[c] += 2;
                            break;
                        ...
                        case 3: /*Two regs write twice*/
                            hdma.hdmaval[c] =
                                    readf(hdma.ibank[c], hdma.ta[c]) | (readf(hdma.ibank[c], hdma.ta[c] + 1) << 8) |
                                    (readf(hdma.ibank[c], hdma.ta[c] + 2) << 16) |
                                    (readf(hdma.ibank[c], hdma.ta[c] + 3) << 24);
                            hdma.ta[c] += 4;
                            break;
                        ...
                        default:
                            printf("Bad HDMA indirect mode %i  count %i dest %02X  data %02X:%04X\n", dma.ctrl[c] & 7,
                                   hdma.hdmacount[c], dma.dest[c], hdma.ibank[c], hdma.ta[c]);
                            dumpregs();
                            exit(-1);
                    }
                } else {
                    switch (dma.ctrl[c] & 7) {
                        ...
                        case 1: /*Two regs write once*/
                            stage = 160;
                            hdma.hdmaval[c] = readf(hdma.srcbank[c], hdma.src[c]) |
                                              (readf(hdma.srcbank[c], hdma.src[c] + 1) << 8);
                            hdma.src[c] += 2;
                            stage = 161;
                            break;
                        case 2: /*One reg write twice*/
                        ...
                        case 3: /*Two regs write twice*/
                            ...
                            hdma.hdmaval[c] = readf(hdma.srcbank[c], hdma.src[c]) |
                                              (readf(hdma.srcbank[c], hdma.src[c] + 1) << 8) |
                                              (readf(hdma.srcbank[c], hdma.src[c] + 2) << 16) |
                                              (readf(hdma.srcbank[c], hdma.src[c] + 3) << 24);
                            hdma.src[c] += 4;
                            stage = 161;
                            break;
                        case 4: /*Four regs*/
...
void dosprites(const int pri, const int ms) {
    ...
    for (c = 127; c >= 0; c--) {
        if (pri == sprites.p[c]) {
            ...
            if ((sprites.x[c] < 264) && (sprites.y[c] < 240) && ((sprites.x[c] + (sprites.sx[c] << 3)) >= 0) &&
                ((sprites.y[c] + (sprites.sy[c] << 3)) >= -16)) /*Clip offscreen sprites*/
            {
                if (sprites.vf[c]) {
                    vend = -8;
                        ...
                        for (x = hstart; x != hend; x += hdir) {
                            if ((x + sprites.x[c] + 16) >= 0 && (x + sprites.x[c] + 24) <= 279) {
                                for (yy = 0; yy < 8; yy++) {
                                    ...
                                    p = (uint32_t *) native_bitmap_pointer(x + sprites.x[c], y + yy + sprites.y[c] + 1);
...
void drawbackground(const int bg, const int pri, const int bg3p, const int ms) {
    ...
    for (line = 0; line < 224; line++) {
        ...
        if (tilemode) {
            ...
            p = (uint32_t *) native_bitmap_pointer(-(lines[line].xs[bg] & 15), line);
            baddr += ylookup[lines[line].ysize[bg]][(lin >> 4) & 63];
            x = (lines[line].xs[bg] >> 4) & 63;
        } else {
            lin = (line + lines[line].ys[bg]) & 511;
            p = (uint32_t *) native_bitmap_pointer(-(lines[line].xs[bg] & 7), line);
...
#define PRI 0x2000
    ...
    if (ppu_w1l > 0 && (ppu_wobjsel & 0x33) == 0x23) {
        for (line = 0; line < 224; line++) {
            uint32_t *p = native_bitmap_pointer(0, line);
            int x;
