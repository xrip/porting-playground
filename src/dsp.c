...
inline signed short decodebrr(int v) {
    ...
    switch (filter) {
        case 0:
            break;
        case 1:
            tempf = (float) lastsamp[0] * ((float) 15 / (float) 16);
            temp += tempf;
            break;
        case 2:
            tempf = ((float) lastsamp[0] * ((float) 61 / (float) 32)) -
                    ((float) lastsamp[1] * ((float) 15 / (float) 16));
            temp += tempf;
            break;
        case 3:
            tempf = ((float) lastsamp[0] * ((float) 115 / (float) 64)) -
                    ((float) lastsamp[1] * ((float) 13 / (float) 16));
            temp += tempf;
            break;
        default:
