...
void dumpregs() {
    ...
    printf("DP=%04X DBR=%04X mode %i  %06X %06X %06X %06X %06X %06X %06X %06X %i ins\n", dp, dbr, mode, oldpc[7],
           oldpc[6], oldpc[5], oldpc[4], oldpc[3], oldpc[2], oldpc[1], oldpc[0], ins);
...
unsigned char readmeml(unsigned char b, unsigned short addr) {
    if (lorom) {
        if (b == 0x70 &&
            sramsize) { addr &= srammask[sramsize]; /*printf("R SRAM %04X = %02X\n",addr,sram[addr]); */return sram[addr]; }
        b &= 0x7F;
        if (b < 0x60) {
            switch (addr & 0xF000) {
                case 0x0000:
...
//        if (stage==10 && pc==0xB8B5) printf("%02X %04X %02X\n",b,addr,val);
    if (lorom) {
        if (b == 0x70 && sramsize) {
            addr &= srammask[sramsize]; /*printf("SRAM %04X=%02X\n",addr,val); */sram[addr] = val;
            return;
        }
        ...
        if (b < 0x60) {
            switch (addr & 0xF000) {
                ...
                case 0x2000:
                    ...
                    return;
                case 0x3000:
                    return; /*TMNT4 writes here*/
                case 0x4000:
                    writeio(addr, val);
                    return;
                case 0x5000:
                case 0x6000:
                    return; /*Pushover writes here*/
                case 0x7000:
                    return; /*Wario's Woods writes here*/
...
void makehiromtable() {
    ...
    for (c = 0; c < 0x40; c++) {
        ...
        mempointv[c + 0x80][0] = mempointv[c + 0x80][4] = mempointv[c + 0x80][5] = mempointv[c + 0x80][6] = mempointv[
                c + 0x80][7] = 1;
...
static inline unsigned short getword() {
    unsigned short tw;
    tw = readmem(pbr, pc) | (readmem(pbr, pc + 1) << 8);
    pc += 2;
    return tw;
}
...
static inline uint32_t indx() {
    uint32_t templ;
    unsigned short t = readmem(pbr, pc) + dp + reg_X.w;
    pc++;
    templ = (readmem(0, t) | (readmem(0, t + 1) << 8) | (dbr << 16));
    return templ;
}

static inline uint32_t indy() {
    uint32_t templ;
    unsigned short t = readmem(pbr, pc) + dp;
    pc++;
    templ = (readmem(0, t) | (readmem(0, t + 1) << 8) | (dbr << 16)) + reg_Y.w;
    return templ;
}

static inline uint32_t indys() {
    uint32_t templ;
    unsigned short t = readmem(pbr, pc) + reg_S.w;
    pc++;
    templ = (readmem(0, t) | (readmem(0, t + 1) << 8) | (dbr << 16)) + reg_Y.w;
    return templ;
}

static inline uint32_t indly() {
    uint32_t templ;
    unsigned short t = readmem(pbr, pc) + dp;
    pc++;
...
/*ADC abs*/   void op6D() {
    templ = _abs();
    tempw = readmemw(templ);
    if (struct_p.d) { ADCBCD16() } else { ADCB16() }
    cycles -= 5;
}

/*ADC abs*/   void op6Dm() {
    templ = _abs();
    temp = readmem(templ >> 16, templ);
    if (struct_p.d) { ADCBCD8() } else { ADCB8() }
    cycles -= 4;
}
...
/*ASL abs*/   void op0E() {
    templ = _abs();
    tempw = readmemw(templ);
    struct_p.c = tempw & 0x8000;
    tempw <<= 1;
    writememw(templ, tempw);
    setzn16(tempw);
    cycles -= 8;
}
...
/*ASL abs*/   void op0Em() {
    templ = _abs();
    temp = readmem(templ >> 16, templ);
    struct_p.c = temp & 0x80;
    temp <<= 1;
    writemem(templ >> 16, templ, temp);
    setzn8(temp);
    cycles -= 6;
}
...
/*BCC*/ void op90() {
    offset = (signed char) readmem(pbr, pc);
    pc++;
    if (!struct_p.c) {
        pc += offset;
        cycles--;
...
/*BEQ*/ void opF0() {
    if (setzf > 0) struct_p.z = 0; else if (setzf < 0) struct_p.z = 1;
    offset = (signed char) readmem(pbr, pc);
    pc++;
    if (struct_p.z) {
        pc += offset;
...
/*BNE*/ void opD0() {
    if (setzf > 0) struct_p.z = 1; else if (setzf < 0) struct_p.z = 0;
    offset = (signed char) readmem(pbr, pc);
    pc++;
    if (!struct_p.z) {
        pc += offset;
...
/*BIT abs*/   void op2C() {
    templ = _abs();
    tempw = readmemw(templ);
    struct_p.z = !(reg_A.w & tempw);
    struct_p.v = tempw & 0x4000;
    struct_p.n = tempw & 0x8000;
    cycles -= 5;
}
...
/*BIT abs*/   void op2Cm() {
    templ = _abs();
    temp = readmem(templ >> 16, templ);
    struct_p.z = !(reg_A.b.l & temp);
    struct_p.v = temp & 0x40;
    struct_p.n = temp & 0x80;
    cycles -= 4;
}
...
/*CMP abs*/   void opCD() {
    templ = _abs();
    tempw = readmemw(templ);
    setzn16(reg_A.w - tempw);
    struct_p.c = (reg_A.w >= tempw);
    cycles -= 5;
}
...
/*CMP abs*/   void opCDm() {
    templ = _abs();
    temp = readmem(templ >> 16, templ);
    setzn8(reg_A.b.l - temp);
    struct_p.c = (reg_A.b.l >= temp);
    cycles -= 4;
}
...
/*CPX abs*/ void opEC() {
    templ = _abs();
    tempw = readmemw(templ);
    setzn16(reg_X.w - tempw);
    struct_p.c = (reg_X.w >= tempw);
    cycles -= 5;
}
...
/*CPX abs*/ void opECx() {
    templ = _abs();
    temp = readmem(templ >> 16, templ);
    setzn8(reg_X.b.l - temp);
    struct_p.c = (reg_X.b.l >= temp);
    cycles -= 4;
}
...
/*CPY abs*/ void opCC() {
    templ = _abs();
    tempw = readmemw(templ);
    setzn16(reg_Y.w - tempw);
    struct_p.c = (reg_Y.w >= tempw);
    cycles -= 5;
}
...
/*CPY abs*/ void opCCx() {
    templ = _abs();
    temp = readmem(templ >> 16, templ);
    setzn8(reg_Y.b.l - temp);
    struct_p.c = (reg_Y.b.l >= temp);
    cycles -= 4;
}
...
/*JML*/ void opDC() {
    templ = jabs();
    pc = readmemw(templ);
    pbr = readmem(templ >> 16, templ + 2);
    cycles -= 6;/* printf("JML %02X %02X %02X\n",readmem(pbr,pc),readmem(pbr,pc+1),readmem(pbr,pc+2)); exit(0);*/}
...
/*JSL*/ void op22() {
    tempw = getword();
    temp = readmem(pbr, pc);
    writemem(0, reg_S.w, pbr);
    writemem(0, reg_S.w - 1, pc >> 8);
    writemem(0, reg_S.w - 2, pc & 0xFF);
    reg_S.w -= 3;
    pc = tempw;
    pbr = temp;
    cycles -= 8;
...
/*JSR*/ void op20() {
    tempw = getword();
    pc--;
    writemem(0, reg_S.w, pc >> 8);
    writemem(0, reg_S.w - 1, pc & 0xFF);
    reg_S.w -= 2;
    pc = tempw;
    cycles -= 6;
}
...
/*LDA abs*/   void opADm() {
    templ = _abs();
    reg_A.b.l = readmem(templ >> 16, templ);
    setzn8(reg_A.b.l);
    cycles -= 4;
}
...
/*LDX imm*/   void opA2() {
    reg_X.b.l = readmem(pbr, pc);
    reg_X.b.h = readmem(pbr, pc + 1);
    pc += 2;
    setzn16(reg_X.w);
    cycles -= 3;
}
...
/*LDX abs*/   void opAEx() {
    templ = _abs();
    reg_X.b.l = readmem(templ >> 16, templ);
    setzn8(reg_X.b.l);
    cycles -= 4;
}
...
/*LDY imm*/   void opA0() {
    reg_Y.b.l = readmem(pbr, pc);
    reg_Y.b.h = readmem(pbr, pc + 1);
    pc += 2;
    setzn16(reg_Y.w);
    cycles -= 3;
}
...
/*LDY abs*/   void opACx() {
    templ = _abs();
    reg_Y.b.l = readmem(templ >> 16, templ);
    setzn8(reg_Y.b.l);
    cycles -= 4;
}
...
/*MVP*/ void op44() {
    ...
    tempb = readmem(pbr, pc + 1);
    pc += 2;
    tempw = readmem(tempb, reg_X.w);
    writemem(temp, reg_Y.w, tempw);
    reg_X.w--;
    reg_Y.w--;
    reg_A.w--;
    if (reg_A.w != 0xFFFF) pc -= 3;
...
/*PEA*/ void opF4() {
    tempw = getword();
    writemem(0, reg_S.w, tempw >> 8);
    writemem(0, reg_S.w - 1, tempw);
    reg_S.w -= 2;
    cycles -= 5;
}
...
/*PHA*/ void op48() {
    writemem(0, reg_S.w, reg_A.b.h);
    writemem(0, reg_S.w - 1, reg_A.b.l);
    reg_S.w -= 2;
    cycles -= 4;
}

/*PHA*/ void op48m() {
    writemem(0, reg_S.w, reg_A.b.l);
    reg_S.w--;
    cycles -= 3;
}
...
/*PHD*/ void op0B() {
    writemem(0, reg_S.w, dp >> 8);
    writemem(0, reg_S.w - 1, dp);
    reg_S.w -= 2;
    cycles -= 4;
}
...
/*PHP*/ void op08() {
    temp = 0;
    if (struct_p.c) temp |= 1;
    if (struct_p.z) temp |= 2;
    if (struct_p.i) temp |= 4;
    if (struct_p.d) temp |= 8;
    if (struct_p.v) temp |= 0x40;
    if (struct_p.n) temp |= 0x80;
    if (struct_p.x) temp |= 0x10;
    if (struct_p.m) temp |= 0x20;
    writemem(0, reg_S.w, temp);
    reg_S.w--;
    cycles -= 3;
}
...
/*PHX*/ void opDA() {
    writemem(0, reg_S.w, reg_X.b.h);
    writemem(0, reg_S.w - 1, reg_X.b.l);
    reg_S.w -= 2;
    cycles -= 4;
}

/*PHX*/ void opDAx() {
    writemem(0, reg_S.w, reg_X.b.l);
    reg_S.w--;
    cycles -= 3;
}
...
/*PHY*/ void op5A() {
    writemem(0, reg_S.w, reg_Y.b.h);
    writemem(0, reg_S.w - 1, reg_Y.b.l);
    reg_S.w -= 2;
    cycles -= 4;
}

/*PHY*/ void op5Ax() {
    writemem(0, reg_S.w, reg_Y.b.l);
    reg_S.w--;
    cycles -= 3;
}
...
/*PLA*/ void op68() {
    reg_A.b.l = readmem(0, reg_S.w + 1);
    reg_A.b.h = readmem(0, reg_S.w + 2);
    reg_S.w += 2;
    setzn16(reg_A.w);
    cycles -= 5;
}
...
/*PLD*/ void op2B() {
    dp = readmem(0, reg_S.w + 1);
    dp |= (readmem(0, reg_S.w + 2) << 8);
    reg_S.w += 2;
    cycles -= 5;
}

/*PLP*/ void op28() {
    temp = readmem(0, reg_S.w + 1);
    reg_S.w++;
    struct_p.c = temp & 1;
    struct_p.z = temp & 2;
    struct_p.i = temp & 4;
    ...
    struct_p.v = temp & 0x40;
    struct_p.n = temp & 0x80;
    struct_p.x = temp & 0x10;
    struct_p.m = temp & 0x20;
    updatemode();
    cycles -= 4;
}
...
/*PLX*/ void opFA() {
    reg_X.b.l = readmem(0, reg_S.w + 1);
    reg_X.b.h = readmem(0, reg_S.w + 2);
    reg_S.w += 2;
    setzn16(reg_X.w);
    cycles -= 5;
}
...
/*PLY*/ void op7A() {
    reg_Y.b.l = readmem(0, reg_S.w + 1);
    reg_Y.b.h = readmem(0, reg_S.w + 2);
    reg_S.w += 2;
    setzn16(reg_Y.w);
    cycles -= 5;
}
...
/*REP*/ void opC2() {
    temp = readmem(pbr, pc);
    pc++;
    if (temp & 1) struct_p.c = 0;
    if (temp & 2) struct_p.z = 0;
    if (temp & 4) struct_p.z = 0;
    if (temp & 8) struct_p.n = 0;
    if (temp & 0x40) struct_p.z = 0;
    if (temp & 0x80) struct_p.n = 0;
    if (!struct_p.e) {
        if (temp & 0x10) struct_p.x = 0;
        if (temp & 0x20) struct_p.m = 0;
    }
    updatemode();
    cycles -= 3;
}
...
/*RTI*/ void op40() {
    ...
    struct_p.x = temp & 0x10;
    struct_p.m = temp & 0x20;
    pc = (readmem(0, reg_S.w + 1) | (readmem(0, reg_S.w + 2) << 8));
    reg_S.w += 2;
    pbr = readmem(0, reg_S.w + 1);
    reg_S.w++;
    cycles -= 7;
    updatemode();
}
...
/*RTL*/ void op6B() {
    pc = (readmem(0, reg_S.w + 1) | (readmem(0, reg_S.w + 2) << 8)) + 1;
    pbr = readmem(0, reg_S.w + 3);
    reg_S.w += 3;
    cycles -= 6;
}
...
/*SBC abs*/   void opED() {
    templ = _abs();
    tempw = readmemw(templ);
    if (struct_p.d) { SBCBCD16() } else { SBCB16() }
    cycles -= 5;
}
...
/*SBC abs*/   void opEDm() {
    templ = _abs();
    temp = readmem(templ >> 16, templ);
    if (struct_p.d) { SBCBCD8() } else { SBCB8() }
    cycles -= 4;
}
...
/*SEP*/ void opE2() {
    temp = readmem(pbr, pc);
    pc++;
    if (temp & 1) struct_p.c = 1;
    if (temp & 2) struct_p.z = 1;
    if (temp & 4) struct_p.z = 1;
    if (temp & 8) struct_p.n = 1;
    if (temp & 0x40) struct_p.z = 1;
    if (temp & 0x80) struct_p.n = 1;
    if (!struct_p.e) {
        if (temp & 0x10) struct_p.x = 1;
        if (temp & 0x20) struct_p.m = 1;
    }
    updatemode();
    cycles -= 3;
}
...
/*STA abs*/   void op8D() {
    templ = _abs();
    writememw(templ, reg_A.w);
    cycles -= 5;
}

/*STA abs*/   void op8Dm() {
    templ = _abs();
    writemem(templ >> 16, templ, reg_A.b.l);
    cycles -= 4;
}
...
/*STX abs*/   void op8E() {
    templ = _abs();
    writememw(templ, reg_X.w);
    cycles -= 5;
}

/*STX abs*/   void op8Ex() {
    templ = _abs();
    writemem(templ >> 16, templ, reg_X.b.l);
    cycles -= 4;
}
...
/*STY abs*/   void op8C() {
    templ = _abs();
    writememw(templ, reg_Y.w);
    cycles -= 5;
}

/*STY abs*/   void op8Cx() {
    templ = _abs();
    writemem(templ >> 16, templ, reg_Y.b.l);
    cycles -= 4;
}
...
/*TSX*/ void opBA() {
    reg_X.w = reg_S.w;
    if (struct_p.x) reg_X.b.h = 0;
    cycles -= 2;
}

/*TXS*/ void op9A() {
    reg_S.w = reg_X.w;
    if (struct_p.e) reg_S.b.h = 1;
    cycles -= 2;
}
